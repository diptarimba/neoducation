			...
			<div class="col">
				...
				<div class="card-body">
					...
					<thead>
						...
						<th>Unpaid Pertemuan</th>
						<th>Paid Pertemuan</th>
						<th>Package</th>
						<th>Action</th>
					</thead>
					<tbody>
						...
						<tr>
							...
							{{-- <td>{{$each -> username}}</td> --}}
							<td>{{$each -> count}}</td>
							<td>{{$each -> unpaid_count}}</td>
							<td>{{$each -> paid_count}}</td>
							<td>{{$each -> package}}</td>
							<td>
                                <div class="row">
                                    <a href="/managesiswa/{{$each->id}}"class="btn col mx-1 my-1 btn-outline-success btn-sm">Detail</a>
                                    <a href="/admin/siswa/invoice/{{$each->id}}"class="btn col mx-1 my-1 btn-outline-warning btn-sm">Invoice</a>
                                </div>
							</td>
						</tr>
						@endforeach
